...
### Errata against v1 of this brief (2026-08-25, ledger 069)

Found by a third reviewing seat. Kept visible rather than silently fixed, per the
programme's rule.

- **§1 said `R` sits a hair *under* `1/24`. It sits a hair *over*.** `1/24 = 0.041666…`,
  `R = 0.041689…`, and `ε = 24R − 1 = +0.000546 > 0` — which this brief's own table
  states three lines below the error. Corrected in place below.
- **§O8's prose said "`d′` and `b′`"** where the table defines `b*`. Same object,
  inconsistent name.
- **`T₂` was used in P6 and never defined.** Now defined in §1.
...
`R` sits a hair **over** `1/24` (`1/24 = 0.041666…`). In March 2026 a closed form was proposed that matched
...
`ε = 24R − 1`. `S(m) = Σ_{k∈ℤ², |k|²=m} (−1)^(k₁)`. **`T₂(m) = Σ_{k∈ℤ², |k|²=m} (−1)^(k₁) k₂²`** (defined in ledger 038; omitted from v1 of this brief).
...
object. *We flagged this as the most likely place our recent work is wrong.*

**UPDATE 2026-08-25 (ledger 069): a reviewing seat thinks Carlson closes it.** Two
functions agreeing at every integer differ by something shaped like `sin(πd)`, which
Carlson's theorem forces to grow like `e^(π|Im d|)` off the real axis. Our
continuation cannot grow vertically: `d` appears only in `t^((d+1)/2−1)` and
`θ₃^(d−j)`, whose **moduli depend only on `Re d`** — an imaginary exponent rotates
phase, not magnitude — and the `1/Γ((d+1)/2)` prefactor decays vertically. Gaps
remaining: analyticity on a right half-plane (dominated convergence), the vertical
bound written out, and polynomial growth along the real axis. **Not proved.** And the
caveat is structural: uniqueness is always relative to a growth class, so `d*` becomes
*"the zero of the unique Carlson-class continuation."*
...
**UPDATE 2026-08-25 (ledger 069): it is known, and the keyword is
"angular lattice sums."** Sums of `cos^n(mθ)/(p₁²+p₂²)^s` over the square lattice,
going back to **Rayleigh 1892**; *Lattice Sums Then and Now* (Borwein–Glasser–
McPhedran–Wan–Zucker, Cambridge 2013), **Chapter 3**, by McPhedran and Zucker. And
`C(1, 4m; s)` obeys the **Riemann Hypothesis iff `ζ(s)·β(s)` does** (arXiv:1007.4111;
Proc. R. Soc. A 467(2133)). The `4m` is our four-fold structure — 0°/45°/90° is the
lowest angular harmonic the square sub-symmetry permits, i.e. their `cos 4θ` object.
**Open: whether the marked, deformed, 3-D version appears in closed form anywhere.**
Also flagged and unverified: whether Ambjørn & Wolfram (1983) computed Casimir
energies on rectangular tori as functions of side ratios — our deformation family at
zeroth angular order.
