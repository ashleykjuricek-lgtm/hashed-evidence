...
// ═══════════════════════════════════════════════════════════════
// EXACT RATIONAL CONSTANTS LAYER (GROUND STATE)
// ═══════════════════════════════════════════════════════════════

interface RationalConstant {
  numerator: bigint;
  denominator: bigint;
}

// Fixed mathematical invariants for the discrete 10-variable system
export const PRISMATIC_LATTICE_CONSTANTS: Record<string, RationalConstant> = {
  // Pure Identity Baseline (1/1)
  IDENTITY_ONE: { numerator: 1n, denominator: 1n },

  // The Canonical Corner (1,1,1) Invariant from the Poisson system
  CORNER_1_1_1: { numerator: -11n, denominator: 1536n },

  // Base Trace Ladder step scale
  TRACE_LADDER_BASE: { numerator: 1n, denominator: 7680n }
};

// Safe helper to convert fractions to decimals ONLY when rendering to pixels
export const rationalToFloat = (c: RationalConstant): number => {
  return Number(c.numerator) / Number(c.denominator);
};
...
// ═══════════════════════════════════════════════════════════════
// SCAR — a correction is ADDED beside the claim, never substituted
// for it. Ledger entry 052: "we change nothing and reveal the scar,
// and the next entry says what we got wrong." Deleting a wrong claim
// destroys the evidence that it was ever believed.
// ═══════════════════════════════════════════════════════════════

function Scar({ tag, children }: { tag: string; children: ReactNode }) {
  return (
    <div className="rounded-lg p-4 mt-5" style={{
      background: `${pink}05`,
      border: `1px solid ${pink}22`,
      borderLeft: `3px solid ${pink}`,
    }}>
      <div style={{
        fontFamily: mono, fontSize: "0.5rem", color: pink,
        letterSpacing: "0.15em", marginBottom: 10,
      }}>
        SCAR &middot; {tag} &middot; 2026-08-25
      </div>
      <div className="space-y-2" style={{
        fontFamily: serif, fontSize: "0.85rem", color: "#b8b6b0", lineHeight: 1.8,
      }}>
        {children}
      </div>
    </div>
...
function TrainingSimulator() {
  ...
  const baseVarRef = useRef(1);
    ...
    // We fetch our exact corner fraction value (-0.00716145...) as a float baseline.
    const invariantOffset = Math.abs(rationalToFloat(PRISMATIC_LATTICE_CONSTANTS.CORNER_1_1_1));

    // Create a 10-vertex signal grounded by our exact rational anchor
    const seed = Array.from({ length: 10 }, (_, i) =>
      i % 2 === 0
        ? 0.75 + (Math.random() * 0.2) - invariantOffset
        : 0.3 + (Math.random() * 0.15) + invariantOffset
    );
  ...
  return (
    <section className="mb-16">
      ...
      <div className="rounded-xl p-6" style={{ background: "rgba(0,0,0,0.3)", border: `1px solid ${purple}15` }}>
        ...
        <Scar tag="CARRIER CLOSURE">
          <p>
            <strong style={{ color: pink }}>The derivation above does not currently typecheck.</strong>{" "}
            It assumes a four-element carrier <span style={{ fontFamily: mono }}>{"{1, 0, −1, ω}"}</span>.
            That carrier is <em>not closed</em> once <span style={{ fontFamily: mono }}>&minus;&omega; &ne; 0</span> &mdash;
            the four-cycle step leaves the set it assumes. Recorded in
            <span style={{ fontFamily: mono, color: dim }}> hashed-evidence/086</span>.
          </p>
          <p>
            The steps are left standing because deleting them would destroy the evidence that they were
            believed. <strong style={{ color: amber }}>What survives is the principle, not the derivation:</strong>{" "}
            when an exact operation leaves the assumed carrier, <em>enlarge the carrier</em> rather than
            coerce the output into an existing slot. Closure by extension, never closure by coercion.
          </p>
        </Scar>

        <Scar tag="UNSUPPORTED">
          <p>
            The italic line above &mdash; that Euler's identity is the <em>smoothed average</em> of
            0<sup>&omega;</sup> = &minus;1 &mdash; <strong style={{ color: pink }}>has no support at any
            status anywhere in the corpus.</strong> It is neither proved, nor observed, nor offered.
            Read it as a conjecture that has never been tested, not as a result.
          </p>
        </Scar>
      </div>
    </section>
...
function CosmologicalTable() {
  ...
  return (
    <section className="mb-16">
      ...
      <Scar tag="INTERPRETIVE &mdash; NOTHING IN THIS TABLE IS DERIVED">
        <p>
          <strong style={{ color: pink }}>Not one row of the table above appears in the sealed corpus at
          any status.</strong> Not proved, not observed, not offered. Light speed as lattice bandwidth,
          mass as &phi;-vertex density, gravity, black holes, entanglement &mdash; these are
          <em> translations of vocabulary</em>, not results.
        </p>
        <p>
          The table is worth reading as what a lattice picture would <em>look like</em> if it held. It is
          not evidence that it does. <strong style={{ color: amber }}>A framework that can restate every
          known physical fact in its own words has not thereby predicted any of them</strong> &mdash;
          restatement is free, and the ledger's own failure mode (F8) is exactly a claim about the world
          whose only support is a fact about the apparatus.
        </p>
      </Scar>
    </section>
  );
  ...
  return (
    <section className="mb-16">
      ...
      <div className="space-y-4">
        ...
        <Scar tag="CAUSAL CLAIM, NO EXPERIMENT">
          <p>
            <strong style={{ color: pink }}>&ldquo;Standard AI hallucinates because it has no Q<sub>&phi;</sub>&rdquo;
            is a causal claim about model behaviour with no experiment behind it.</strong>{" "}
            Nothing in this corpus tests it. Read it as a hypothesis about why averaging might cost
            something, not as a finding about why models confabulate.
          </p>
          <p>
            The narrower statement <em>is</em> supported, and it is the one worth keeping: an update rule
            that contracts toward the mean destroys structure, for any input, by construction. Whether
            that is what happens inside a trained network is a separate question and an open one.
          </p>
        </Scar>
      </div>
    </section>
      ...
      <div className="max-w-4xl mx-auto px-6 pt-24 pb-16">
        ...
        <div className="mb-16">
          ...
          <div style={{ maxWidth: 600 }}>
            <Scar tag="HALF OF THIS IS TESTED">
              <p>
                <strong style={{ color: green }}>&pi;: supported, in one specific sense.</strong> Counting
                ideals in four number-worlds with every formula refused, &pi; turned out to have
                <em> no integer seed</em> &mdash; it exists only as the limit of a count divided by a
                scale, which is what a smoothing <em>is</em>. The golden world's constant, by contrast,
                is an integer pair you can find by searching, wearing a logarithm.
              </p>
              <p>
                <strong style={{ color: pink }}><em>e</em>: never examined.</strong> It has not been
                tested here at any status. The sentence asserts two things and has checked one.
              </p>
            </Scar>
          </div>
        ...
        <div className="text-center pb-16 mt-8">
          ...
          <div className="text-left mx-auto mt-6" style={{ maxWidth: 620 }}>
            <Scar tag="&pi; CANNOT BE BANNED">
              <p>
                The clean-room run excluded &pi; from the setup entirely &mdash; no class-number formula,
                no <span style={{ fontFamily: mono }}>(2&pi;)</span> anywhere, just integer pairs counted
                under a bound. <strong style={{ color: amber }}>&pi; walked in by itself, as an
                area</strong>, forced by the region being round. Lattice points in a region approach that
                region's area; the area of a round region is &pi;. It is not imported and it cannot be
                refused.
              </p>
              <p>
                <strong style={{ color: green }}>What &pi; cannot do is seed anything.</strong> No search
                over integers produces it. That is the real asymmetry, and it is sharper than banning:
                the golden world's constant has an exact integer seed &mdash; the pair
                <span style={{ fontFamily: mono }}> (1,1) </span> &mdash; wearing a logarithm. &pi; has
                none, and exists only as a limit of a count over a scale.
              </p>
              <p style={{ color: dim }}>
                Also worth stating so it is not claimed later: measuring
                <span style={{ fontFamily: mono }}> 0.78540300 </span> is consistent with &pi;/4 and with
                infinitely many other numbers. <em>No amount of counting can promote a decimal to a named
                constant.</em> What identifies it is the area argument &mdash; a proof, not the
                measurement. Sealed as <span style={{ fontFamily: mono }}>hashed-evidence/087</span>.
              </p>
            </Scar>
          </div>
