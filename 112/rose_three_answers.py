...
def lattice_points(proj_fn, D, N, reach):
    """The unique projected points of the [-N, N]^D grid whose PERP image lies within `reach`
    of the origin -- built one slab of the first coordinate at a time so large N fits in memory.
    Returns (PAR, PERP) after the same 1e-8 PAR dedup as the original recipe.  Equivalent to
    grid -> project -> dedup -> prefilter, because duplicates share PERP (asserted)."""
    Ppar, Pperp = proj_fn()
    c = np.arange(-N, N + 1, dtype=float)
    sub = np.array(np.meshgrid(*([c] * (D - 1)), indexing="ij")).reshape(D - 1, -1).T
    keep_par, keep_perp = [], []
    for x0 in c:
        g = np.column_stack([np.full(len(sub), x0), sub])
        PERP = g @ Pperp.T
        m = np.hypot(PERP[:, 0], PERP[:, 1]) < reach
        if m.any():
            keep_par.append((g[m] @ Ppar.T))
            keep_perp.append(PERP[m])
    PAR = np.concatenate(keep_par); PERP = np.concatenate(keep_perp)
    key = np.round(PAR / 1e-8).astype(np.int64)
    _, first, inverse = np.unique(key, axis=0, return_index=True, return_inverse=True)
    inverse = inverse.ravel()
    spread = np.zeros(len(first))
    np.maximum.at(spread, inverse, np.abs(PERP - PERP[first][inverse]).max(axis=1))
    assert spread.max() < 1e-9, f"duplicate PAR points with different PERP: {spread.max()}"
    return PAR[first], PERP[first]
...
class RoseZeta(Expr):
    ...
    def __init__(self, proj_fn, half, D, N, name="theta", window="poly"):
        ...
        PAR, PERP = lattice_points(proj_fn, D, N, reach)
        self.PAR, self.PERP = PAR, PERP

        # drop the origin (zeta does)
