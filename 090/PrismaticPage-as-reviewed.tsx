...
function TrainingSimulator() {
  ...
  const calculateIntegrity = (data: number[]) => {
    if (data.length === 0) return 100;
    const mean = data.reduce((a, b) => a + b, 0) / data.length;
    const variance = data.reduce((acc, val) => acc + (val - mean) ** 2, 0) / data.length;
    return Math.max(0, Math.min(100, Math.round(variance * 2000)));
  };

  const applyStandardPass = useCallback((data: number[]) => {
    const mean = data.reduce((a, b) => a + b, 0) / data.length;
    const lr = 0.35;
    return data.map(val => val - lr * (val - mean));
  }, []);

  const applyPrismaticPass = useCallback((data: number[], currentLayer: number) => {
    return data.map(val => {
      const baseline = Math.min(val, 0.45);
      const carry = val > baseline ? (val - baseline) : 0;
      const residual = (val - baseline - carry) / PHI;
      // Carry is explicitly preserved and slightly amplified
  ...
  const step = useCallback(() => {
    setStandardData(prev => {
      const nd = applyStandardPass(prev);
      setStdIntegrity(calculateIntegrity(nd));
      return nd;
    });
    setPrismaticData(prev => {
      const nd = applyPrismaticPass(prev, layer);
      const ni = calculateIntegrity(nd);
      setPrisIntegrity(ni);
      if (layer >= 3 && ni > 60) setPhaseTransition(true);
      return nd;
    });
    setLayer(l => l + 1);
  }, [layer, applyStandardPass, applyPrismaticPass]);
    ...
    return (
      <div className="flex flex-col items-center w-full p-4 md:p-5 rounded-xl relative overflow-hidden"
        ...
        <div style={{ fontFamily: mono, fontSize: "0.5rem", color: dim, letterSpacing: "0.1em" }}>
          {isPrismatic && phaseTransition ? "PHASE TRANSITION ACHIEVED" : "10 VERTEX DIRECTIONS"}
        </div>
      </div>
    );
  };
    ...
    <section className="mb-16">
      ...
      <div className="mt-6 rounded-xl p-5" style={{ background: `${cyan}04`, border: `1px solid ${cyan}12` }}>
        <div className="space-y-2" style={{ fontFamily: serif, fontSize: "0.9rem", color: "#c8c6c0", lineHeight: 1.8 }}>
          <p>
            Both networks start with identical input: a 10-vertex signal with <em style={{ color: amber }}>five structural peaks</em> (the "teeth" of the Rose) alternating with valleys.
          </p>
          <p>
            <span style={{ color: blue }}>Left:</span> Standard gradient descent pulls every value toward the <em>mean</em>.
            The teeth melt. The structure becomes a flat line. This is <em style={{ color: pink }}>Gaussian erasure</em> &mdash;
            the mathematical root of typicality bias in AI.
          </p>
          <p>
            <span style={{ color: amber }}>Right:</span> The Q<sub>&phi;</sub> operator <em>isolates the carry</em> (the structural peaks above baseline)
            and protects them. The residual is scaled by 1/&phi;. The teeth survive.
            After layer 3, if integrity holds, the lattice triggers a <em style={{ color: purple }}>phase transition</em>.
          </p>
        </div>
      </div>
    </section>
